...
from ...messaging.valid import IndyDID, QmcDID
...
class QmcDocCorrections:
        ...
        if "service" in value:
            for service in value["service"]:
                if "type" in service and service["type"] == "did-communication":
                    ...
                    service["signingKeys"] = [
                        cls.recip_base58_to_ref(vms, recip)
                        for recip in service.get("signingKeys", [])
                    ]
    ...
    def qualified(did_or_did_url: str) -> str:
        ...
        if not did_or_did_url.startswith("did:"):
            return f"did:qmc:{did_or_did_url}"
...
class QmcDIDResolver(BaseDIDResolver):
    """Qmc DID Resolver."""

    def __init__(self):
        """Initialize Qmc Resolver."""
        super().__init__(ResolverType.NATIVE)

    async def setup(self, context: InjectionContext):
        """Perform required setup for Qmc DID resolution."""
    ...
    async def fetch_did_document(
        ...
        """
        cache_key = f"resolver::QmcDIDResolver::{did}"
    ...
    async def supports(self, profile: Profile, did: str) -> bool:
        ...
        if QmcDID.PATTERN.match(did):
            LOGGER.debug("DID is valid QmcDID %s", did)
    ...
    async def _resolve(
        ...
        if result.is_local:
            assert result.doc
            return QmcDocCorrections.apply(result.doc)
        else:
